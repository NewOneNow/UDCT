...
# -----------------------------
# HDBSCAN 聚类辅助函数
def hdbscan_batch(x, k_max=None, min_cluster_size=2, downscale=None):
    B, C, H, W = x.shape
    k_max = C // 2 if k_max is None else k_max

    fused = x.reshape(B * C, H, W)

    fused = F.adaptive_avg_pool2d(fused, (H // 2, W // 2))
    h, w = fused.shape[1], fused.shape[2]

    # (C, HW)  每一行是一个通道的展平特征
    feats = fused.view(C, -1).detach().cpu().numpy()

    # ★ 保护：通道数不足时直接返回「随机单簇」+ 全噪声
    if feats.shape[0] < min_cluster_size:
        labels_all = torch.full((C,), -1, dtype=torch.long, device=x.device)
        noise = x                                # 把全部通道都当噪声
        return labels_all, noise

    clusterer = HDBSCAN(min_cluster_size=min_cluster_size, min_samples=2)
    labels_all = clusterer.fit_predict(feats)   # (C,)
    labels_all = torch.from_numpy(labels_all).to(x.device).long()

    mask = labels_all.eq(-1)                   # 噪声通道
    noise = x[:, mask, :]                      # (B, k, H, W)
    return labels_all, noise


def hdbscan(x, k_max=None, min_cluster_size=2, downscale=None):

    B, C, H, W = x.shape
    fused = F.adaptive_avg_pool2d(x, (H // downscale, W // downscale))
    h, w = fused.shape[1], fused.shape[2]
    # (C, HW)  每一行是一个通道的展平特征
    feats = fused.view(C, -1).detach().cpu().numpy()
...
class LabelRestrictedSelfAttention(nn.Module):
    def __init__(self, C, num_heads=4):
        super().__init__()
        #assert C % num_heads == 0, "L 必须能被 num_heads 整除"
        self.num_heads = num_heads

        # qkv 投影，输入 (B, N, L)，输出 (B, N, 3L)
        self.qkv = nn.Conv2d(C,3*C*num_heads,1,1,groups=C)#nn.Linear(L, 3 * L, bias=False)
    ...
    def forward(self, x, labels):
        ...
        """
        B, C, H, W = x.shape
        qkv = self.qkv(x)  # (B, 3*Heand*C, H, W )
        q,k,v = qkv.chunk(3, dim=1)
        #q, k, v = torch.chunk(qkv.view(B,N,L), 3, dim=-1)  # (B, N, L) each
        # reshape 成 (B, num_heads, N, head_dim)
        q = q.view(B, self.num_heads, C, -1)
        k = k.view(B, self.num_heads, C, -1)
        v = v.view(B, self.num_heads, C, -1)

        mask = labels != -1  # (C,) bool型
        ...
        for i in range(self.num_heads):
            ...
            ki = k[:,i,:,:]
            vi = v[:,i,:,:]
            s_out = torch.zeros_like(x)
            for j in unique_labels:
                mask = labels == j
                q_m = qi[:, mask, :]  # (B, k, HW)
                k_m_t = ki[:, mask, :].transpose(1,2)  # (B, HW, k)
                v_m = vi[:, mask, :]  # (B, k, HW)
                weights = F.softmax(torch.bmm(q_m,k_m_t),dim=-1)  # B,k,k
                xi_att = torch.bmm(weights,v_m).view(B,-1,H,W)  # B,k,HW
                s_out[:, mask, :,:] = xi_att
            final_out = final_out + s_out
        return final_out
...
class ClusterLocalAtt(nn.Module):
    def __init__(self, H, C, num_heads=4, downscale=4,min_cluster_size=2):
        ...
        # 反卷积恢复到原始分辨率
        self.bais = nn.Conv1d(self.k_max, C, 1,1,groups=self.k_max)
    ...
    def forward(self, x):
        ...
        x: B,C,H,W
        labels: C
        noise_feats: B,i,H//down,W//down
        """
        B, C, H, W = x.shape
        labels, noise_feats = hdbscan_batch(x, downscale=self.downscale,k_max=self.k_max, min_cluster_size=self.min_cluster_size)
        x_down = self.down(x) # B,C,H//down,W//down
        out = self.attn(x_down, labels) # B,C,H//down,W//down
        out = F.interpolate(out, scale_factor=self.downscale, mode='bilinear', align_corners=False)  # B,C,H,W
        if noise_feats.size(1) == 0:
            noise_feats = torch.zeros(B, 1, H, W).to(out.device)
        similar = feature_similarity(out, noise_feats).unsqueeze(-1).unsqueeze(-1)  # B,C,1,1
        out = out+(1 - similar) * x # B,C,H,W

        return out, noise_feats
